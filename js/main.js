...
function initSigma(config) {
	...
	if (config.sigma && config.sigma.drawingProperties)
        ...
        defaultHoverLabelBGColor: "#4D82B8",
...
function nodeActive(a) {
	...
	if (groupByDirection) {
		...
		f.push("<h2>Двојни врски (" + size + ")</h2>");
		(size>0)? f=f.concat(createList(mutual)) : f.push("Нема двојни врски<br>");
		size=Object.size(incoming);
		f.push("<h2>Следбеници (" + size + ")</h2>");
		(size>0)? f=f.concat(createList(incoming)) : f.push("Нема следбеници<br>");
		size=Object.size(outgoing);
		f.push("<h2>Следени (" + size + ")</h2>");
		(size>0)? f=f.concat(createList(outgoing)) : f.push("Не следи никого<br>");
	} else {
    ...
    $GP.info_data.show();
    $GP.info_p.html("Парцијални врски:");
...
function showCluster(a) {
    ...
    if (b && 0 < b.length) {
        ...
        $GP.info_data.hide();
        $GP.info_p.html("Членови на група:");
        $GP.info_link.find("ul").html(f.join(""));
        $GP.info.animate({width:'show'},350);
        $GP.search.clean();
		$GP.cluster.hide();
